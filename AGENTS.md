...
Part of the IGambling platform — see the parent `AGENTS.md` at `/IGambling/AGENTS.md` for full platform context.
...
Entity ↔ domain conversion via mapper extension functions (`object XMapper { fun XEntity.toDomain(): X }`). ResultRow extensions use distinct names (`toCasinoProvider`, `toAggregator`, etc.) to avoid `toDomain` collisions when one mapper composes another — see `.Codex/rules/mapper-conventions.md`.
...
See `.Codex/rules/exposed-database.md` for detailed Exposed ORM conventions.
...
- DTOs in `dto/` subdirectory as `<name>.dto.proto` (see `.Codex/rules/proto-dto.md`)
...
See `.Codex/skills/add-aggregator.md` for the step-by-step guide. See `.Codex/agents/seed-collections.md` for the collection seeding agent.
