...
class SyncAggregatorUsecase(
    private val aggregatorFabric: IAggregatoryFactory,
    ...
    private suspend fun process(aggregator: Aggregator) = coroutineScope {
        val gameAdapter = aggregatorFabric.createGameAdapter(aggregator)
        ...
        for (aggregatorGame in aggregatorGamesAsync.await()) {
            var variant = variantsAsync.await()
                .find { it.symbol == aggregatorGame.symbol && it.integration == aggregator.integration }
